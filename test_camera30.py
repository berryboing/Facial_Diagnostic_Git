...
def show_pairing_qr(master_root, session_id):
    ...
    # --- THE FIX: THREAD-SAFE POLLING ---
    # 1. Create a shared dictionary to act as our thread-safe switch
    ...
    def on_snapshot(doc_snapshot, changes, read_time):
        for doc in doc_snapshot:
            if doc.exists and doc.to_dict().get('status') == 'connected':
                # Grab the email that the Web App just sent over
                global current_user_email
    ...
    # 3. Tkinter Main Thread (Checks the switch every 500ms)
    def check_connection():
        if connection_status["is_connected"]:
            # Now safely operating on the main thread!
            status_label.config(text="✅ Connected successfully! Starting...", fg="#27AE60")
            window.after(1000, window.destroy)
        else:
            # If not connected yet, queue this check to run again in 500ms
...
def process_and_upload(master_root, clean_frame, face_rgb, session_id, model, device, transform):
    ...
    # ==========================================
    time.sleep(5) # Pauses the screen for 3 seconds before moving to results
...
def show_results_and_save(master_root, dep_score, anx_score, str_score, session_id):
    ...
    try:
        ...
        doc_ref.set({
            ...
            'email': globals().get('current_user_email', 'No email'), # Saves the actual user email!
