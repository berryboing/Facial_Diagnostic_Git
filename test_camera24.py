...
def show_pairing_qr(master_root, session_id):
    ...
    # Replaced tap text with a waiting indicator
    ...
    # Create a document for this specific session in Firestore
    ...
    # This callback fires instantly whenever the document changes in the cloud
    def on_snapshot(doc_snapshot, changes, read_time):
        for doc in doc_snapshot:
            if doc.exists and doc.to_dict().get('status') == 'connected':
                # Use window.after to safely update UI on the main thread
                window.after(0, lambda: status_label.config(text="✅ Connected successfully! Starting...", fg="#27AE60"))
                # Use window.after to safely destroy the window on the main thread
    ...
    # Stop listening once the window closes to save memory
...
def main():
    ...
    terminate_app = False # Global kill switch
    ...
    while not terminate_app:
        ...
        # 1. Generate Session & Show Pairing
        current_session_id = str(uuid.uuid4())
        show_pairing_qr(master_root, current_session_id)

        # 2. Show Privacy Notice
        show_privacy_notice(master_root)

        # --- INNER LOOP: Active scanning session ---
        session_active = True
        while session_active and not terminate_app:
            ...
            while True:
                ...
                # --- 1. Draw "Tap to Scan" (Centered, Outlined) ---
                ...
                # --- 2. Draw Error Overlay (Bottom Centered, Outlined, Smaller) ---
                if show_warning:
                ...
                # Check for keyboard kill switch (ESC or 'q')
