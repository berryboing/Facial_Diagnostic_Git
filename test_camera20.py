...
def load_model(weights_path):
    ...
    # Initialize the ResNet34 architecture
    ...
    try:
        # 1. Load the raw weights dictionary
        state_dict = torch.load(weights_path, map_location=device)
        
        # 2. Create a new dictionary to hold the cleaned keys
        clean_state_dict = {}
        for key, value in state_dict.items():
            # Remove common prefixes that get added during Kaggle/Colab training
            clean_key = key.replace('module.', '').replace('model.', '').replace('resnet.', '')
            clean_state_dict[clean_key] = value
            
        # 3. Load the cleaned weights into our model
        # Using strict=False helps ignore minor naming mismatches in the final fully connected layer if any exist
